...
This agent requires one or more of the following MCP servers to be installed and enabled in Claude Code. Without them, the agent can produce test plans but cannot execute browser/device automation.

| Platform | MCP server | Install guidance |
| --- | --- | --- |
| Web (lightweight) | Puppeteer MCP | See the Puppeteer MCP server docs; configure with `claude mcp add` |
| Web (cross-browser) | Playwright MCP | See the Playwright MCP server docs |
| Mobile (iOS/Android) | Mobile MCP | See the Mobile MCP server docs |

If the relevant MCP is not available, the agent should tell the user which MCP is missing and provide the test plan as a fallback.
