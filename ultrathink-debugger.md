...
> ⚠️ **Tool footprint:** this agent is granted `Bash` so it can run reproductions, inspect logs, attach debuggers, and query external services. Review the `tools:` frontmatter before enabling in an environment where arbitrary command execution is a concern.
